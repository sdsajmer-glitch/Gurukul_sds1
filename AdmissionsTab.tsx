...
const AdmissionsTab: React.FC<{ branchId?: number | null }> = ({ branchId }) => {
    ...
    const fetchApplicants = useCallback(async () => {
        // branchId can be null for Head Office admins viewing all branches
        setLoading(true);
        setFetchError(null);
        try {
            // If branchId is null/undefined, pass null to fetch all authorized records
            const targetBranchId = (branchId === null || branchId === undefined) ? null : Number(branchId);

            const { data, error } = await supabase.rpc('get_admissions_v2', {
                p_branch_id: targetBranchId
            });

            if (error) throw error;

            // Filter out internal enquiry states that shouldn't show in the primary Admissions Vault
