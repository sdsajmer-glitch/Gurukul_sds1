-- ==========================================
-- MASTER_FINANCE_RESTORATION_V39_ULTIMATE_AMBIGUITY_FIX
-- Version: 39.0.1 (ULTIMATE)
-- Description: Absolute resolution for "profile_photo_url is ambiguous" across ALL finance nodes.
-- Targets: Finance Center, Parent Portal, Identity Registry.
-- ==========================================
...
BEGIN
    ...
    -- student_fee_accounts (standard placeholder if missing)
...
-- [4] REBUILD: get_parent_authorized_nodes (The Authorization Engine)
...
BEGIN
    ...
    SELECT 
        ...
        s.display_name::text,
        -- Ambiguity Resolve: Explicit Prefix
...
-- [5] REBUILD: get_parent_linked_students_finance_v3 (The Parent Dashboard Feed)
...
-- [6] REBUILD: get_student_fee_summary_all (The Admin Dashboard Table)
...
BEGIN
    ...
    SELECT 
        ...
        -- ULTIMATE UNAMBIGUOUS PHOTO FETCH
        COALESCE(
            p.profile_photo_url, 
...
-- [7] REBUILD: get_student_financial_node (The Detail View Service)
...
SELECT 'SUCCESS: Finance Ambiguity V39 (ULTIMATE) Applied. Handshake synchronized across Parent & Admin portals.' as status;
