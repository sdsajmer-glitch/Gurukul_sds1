...
-- We use CASCADE to automatically remove dependent triggers and constraints.
...
DROP TABLE IF EXISTS public.transport_profiles CASCADE; -- Existing

-- [In CREATE TABLES section]
...
CREATE TABLE public.share_codes (
    ...
    admission_id UUID REFERENCES public.admission_applications(id),
    status TEXT DEFAULT 'Active',
...
CREATE TABLE public.admission_documents (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    admission_id UUID REFERENCES public.admission_applications(id) ON DELETE CASCADE,
    document_name TEXT NOT NULL,
    file_path TEXT,
    status TEXT DEFAULT 'Pending',
...
-- Since we don't have a reliable email link in the admission_applications table for 'parent_user_id' yet, we'll rely on a direct link or fallback to email matching (weak link) or a junction table. for now, let's assume 'parent_email' matches 'email' in profiles.
-- BETTER: We should rely on `admission_applications` having a `parent_id` if we want strict linking.
-- FOR NOW: We will use the `parent_email` from `admission_applications` matching the `email` of the logged-in parent.
...
BEGIN
    ...
    IF v_admission IS NULL THEN
        SELECT * INTO v_admission FROM public.admission_applications WHERE student_user_id = p_student_id LIMIT 1;
    END IF;
