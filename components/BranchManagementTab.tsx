...
const BranchCard: React.FC<{
    ...
    const isLinked = branch.status === 'Active' || branch.status === 'Linked';
    const isProvisioned = branch.status === 'Pending' || (!isLinked && !!branch.access_key);
    ...
    return (
        <div className="group relative bg-card border border-border/60 hover:border-primary/40 rounded-[2.5rem] p-8 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 flex flex-col min-h-[460px] overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-8">
                <div className={`p-4 rounded-2xl shadow-inner transition-all duration-500 ${isLinked ? 'bg-emerald-500 text-white shadow-emerald-500/20' :
                ...
                {isProvisioned && !isLinked ? (
                    <div className="space-y-4 animate-in fade-in duration-500 relative z-10">
                        ...
                        <div className="flex flex-col gap-3">
                            <div className="flex items-center justify-between px-1">
                                <span className="text-[9px] text-white/30 font-bold uppercase tracking-widest">Administrator: {branch.admin_email || 'Pending'}</span>
                            </div>
                        </div>
                    </div>
                ) : (
                    <div className="py-4 text-center space-y-5 animate-in zoom-in-95 duration-500 relative z-10">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-white/30 mt-1 uppercase font-bold tracking-widest">{branch.admin_email}</p>
                        </div>
                    </div>
                )}
            ...
            <div className="mt-8 pt-8 border-t border-border/50 flex flex-col gap-6">
                <div className="flex justify-between items-end">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="flex -space-x-1.5">
                            {[1, 2, 3].map(i => {
                                const stepActive = isLinked ? true : (isProvisioned ? i <= 2 : i <= 1);
                                return (
                ...
                <div className="flex gap-2 h-1.5 w-full bg-muted/40 rounded-full overflow-hidden p-0.5 shadow-inner">
                    ...
                    <div className={`flex-1 rounded-full transition-all duration-700 ${isProvisioned || isLinked ? 'bg-primary/80' : 'bg-transparent'}`}></div>
                ...
                <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-black text-primary uppercase tracking-widest">{isLinked ? 'Synchronized' : 'Provisioning'}</span>
                </div>
            </div>
        </div>
    );
