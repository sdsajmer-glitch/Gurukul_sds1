...
const ONBOARDING_STEPS = ['Plan', 'Profile', 'Branches & Admins'];
...
const OnboardingFlow: React.FC<OnboardingFlowProps> = ({ profile, onComplete, onStepChange, onboardingStep }) => {
    ...
    useEffect(() => {
        ...
        if (profile.role === BuiltInRoles.SCHOOL_ADMINISTRATION) {
            ...
            if (dbStep && ['profile', 'pricing', 'branches'].includes(dbStep)) {
                ...
                setStep('pricing'); // Default to pricing first
            }
        } else {
    ...
    const handleStepAdvance = async () => {
        ...
        try {
            ...
            if (step === 'pricing') {
                // Update database to mark plan selected and move to profile
                ...
                console.log('✅ Advanced to profile step');
                if (isMounted.current) setStep('profile');
    ...
    const handleBack = async () => {
        ...
        if (selectedRole === BuiltInRoles.SCHOOL_ADMINISTRATION) {
            setLoading(true);
            try {
                // Reverse Order: Branches -> Profile -> Pricing -> Role
                if (step === 'branches') {
    ...
    switch (step) {
        case 'role':
            ...
            break;
        case 'profile':
            // Move profile step to be second
