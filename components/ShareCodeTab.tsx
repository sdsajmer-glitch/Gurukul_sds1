    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-white uppercase">Access Codes</h2>
                    <p className="text-white/40 mt-1">Securely share your child's profile with school administrators.</p>
                </div>
                <button onClick={fetchData} className="text-sm font-medium text-primary hover:text-primary/80 flex items-center gap-2 bg-white/5 px-4 py-2 rounded-xl border border-white/5 hover:bg-white/10 transition-all shadow-sm">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                <div className="lg:col-span-5 space-y-6">
                    <div className="bg-[#0c0d12] p-6 rounded-2xl shadow-sm border border-white/5">
                        <div className="mb-6">
                            <h3 className="font-bold text-lg text-white">Create New Code</h3>
                            <p className="text-sm text-white/40">Generate a temporary access key.</p>
                        </div>
                        <form onSubmit={handleGenerateCode} className="space-y-5">
                            <div>
                                <label className="block text-sm font-semibold text-white/60 mb-2">Select Child Profile</label>
                                <div className="relative">
                                    <select
                                        value={selectedAdmission}
                                        onChange={(e) => setSelectedAdmission(e.target.value)}
                                        required
                                        className="w-full p-3 bg-white/5 border border-white/10 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all appearance-none text-sm text-white"
                                    ...
                                    >
                                        ...
                                        {myApplications.map(app => <option key={app.id} value={app.id} className="bg-[#0c0d12]">{app.applicant_name} (Grade {app.grade})</option>)}
                                    </select>
                                    <div className="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none text-white/20">
                                ...
                                {myApplications.length === 0 && !loading && (
                                    <p className="text-xs text-red-400 mt-2 font-medium bg-red-400/10 p-2 rounded-lg">Please add a child in the 'My Children' tab first.</p>
                                )}
                            </div>

                            <div>
                                <label className="block text-sm font-semibold text-white/60 mb-3">Access Type</label>
                                <div className="grid grid-cols-2 gap-4">
                                    <button
                                        type="button"
                                        onClick={() => setCodeType('Enquiry')}
                                        className={`relative p-4 rounded-xl border-2 text-left transition-all duration-200 group ${codeType === 'Enquiry' ? 'border-primary bg-primary/10 ring-1 ring-primary/20' : 'border-white/5 bg-white/5 hover:border-white/10 hover:bg-white/[0.08]'}`}
                                    >
                                        <div className={`w-4 h-4 rounded-full border-2 absolute top-4 right-4 flex items-center justify-center ${codeType === 'Enquiry' ? 'border-primary' : 'border-white/10'}`}>
                                            {codeType === 'Enquiry' && <div className="w-2 h-2 bg-primary rounded-full" />}
                                        </div>
                                        <span className="block font-bold text-sm mb-1 text-white">Enquiry</span>
                                        <span className="block text-[10px] text-white/40 group-hover:text-white/60">For initial questions & visits</span>
                                    </button>
                                    <button
                                        type="button"
                                        onClick={() => setCodeType('Admission')}
                                        className={`relative p-4 rounded-xl border-2 text-left transition-all duration-200 group ${codeType === 'Admission' ? 'border-indigo-500 bg-indigo-500/10 ring-1 ring-indigo-500/20' : 'border-white/5 bg-white/5 hover:border-white/10 hover:bg-white/[0.08]'}`}
                                    >
                                        <div className={`w-4 h-4 rounded-full border-2 absolute top-4 right-4 flex items-center justify-center ${codeType === 'Admission' ? 'border-indigo-500' : 'border-white/10'}`}>
                                            {codeType === 'Admission' && <div className="w-2 h-2 bg-indigo-500 rounded-full" />}
                                        </div>
                                        <span className="block font-bold text-sm mb-1 text-white">Admission</span>
                                        <span className="block text-[10px] text-white/40 group-hover:text-white/60">Full documents for review</span>
                                    </button>
                                </div>
                            </div>

                            <div>
                                <label className="block text-sm font-semibold text-white/60 mb-2">Note <span className="font-normal text-white/20">(Optional)</span></label>
                                <input
                                    type="text"
                                    placeholder="e.g. For Principal meeting"
                                    value={purpose}
                                    onChange={(e) => setPurpose(e.target.value)}
                                    className="w-full p-3 bg-white/5 border border-white/10 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-sm text-white"
                                />
                            </div>
                            ...
                            <button
                                ...
                                className="w-full py-3.5 bg-primary hover:bg-primary/90 text-white font-bold rounded-xl shadow-lg shadow-primary/25 hover:shadow-primary/40 transition-all transform active:scale-[0.98] flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none"
                            >
                                {generating ? <Spinner size="sm" className="text-white" /> : 'Generate Secure Code'}
                            </button>
                        </form>
                        {error && !generatedCode && (
                            <div className="mt-4 p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-center animate-in fade-in zoom-in-95">
                                <p className="text-sm text-red-500 font-semibold">Error</p>
                                <p className="text-xs text-red-400 mt-1">{error.replace('Failed to generate code:', '')}</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="lg:col-span-7 space-y-8">
                    {generatedCode && (
                        <div className="animate-in fade-in slide-in-from-top-4 duration-500">
                            <div className="bg-[#0c0d12] text-white rounded-[2rem] shadow-2xl overflow-hidden border border-white/10 relative">
                                <div className="p-10 pb-6 text-center relative z-10 bg-white/[0.02]">
                                    <div className="w-20 h-20 bg-emerald-500/20 rounded-full flex items-center justify-center mx-auto mb-6 shadow-[0_0_40px_rgba(16,185,129,0.2)] border border-emerald-500/30">
                                        <CheckIcon className="w-10 h-10 text-emerald-500" />
                                    </div>
                                    <h3 className="text-3xl font-black mb-1 text-white uppercase tracking-tighter">Code Generated!</h3>
                                    <p className="text-white/40 text-sm font-medium">Share this securely with institutional nodes.</p>
                                </div>
                                <div className="relative flex items-center justify-between px-10">
                                    <div className="w-8 h-8 bg-background rounded-full -ml-[60px] border border-white/5 shadow-inner"></div>
                                    <div className="flex-1 border-b-2 border-dashed border-white/10 mx-6 opacity-30"></div>
                                    <div className="w-8 h-8 bg-background rounded-full -mr-[60px] border border-white/5 shadow-inner"></div>
                                </div>
                                <div className="p-10 pt-6 relative z-10">
                                    <div
                                        className="border-2 border-dashed border-white/10 bg-white/[0.01] rounded-[2rem] p-10 text-center mb-8 relative group cursor-pointer transition-all hover:border-primary/40 hover:bg-white/[0.03]"
                                        onClick={() => handleCopyCode(generatedCode)}
                                    >
                                        <p className="text-[10px] text-white/20 uppercase tracking-[0.4em] font-black mb-4">Verification Sequence</p>
                                        <div className="text-5xl sm:text-7xl font-mono font-black text-primary tracking-[0.2em] select-all drop-shadow-[0_0_20px_rgba(var(--primary-rgb),0.3)]">{generatedCode}</div>
                                        <div className="absolute inset-0 flex items-center justify-center bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-[2rem] backdrop-blur-md">
                                            <span className="text-white font-black text-xs uppercase tracking-widest flex items-center gap-3">
                                                <CopyIcon className="w-5 h-5" /> Click to Copy Sequence
                                            </span>
                                        </div>
                                    </div>
                                    <div className="flex flex-col sm:flex-row gap-4">
                                        <button
                                            type="button"
                                            onClick={() => handleCopyCode(generatedCode)}
                                            className={`flex-1 py-4 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-3 ${isCopied ? 'bg-emerald-600 text-white cursor-default ring-4 ring-emerald-500/20' : 'bg-primary text-white shadow-xl shadow-primary/20 hover:bg-primary/90'}`}
                                            disabled={isCopied}
                                        >
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-6">
                            <h3 className="font-black text-xs uppercase tracking-[0.3em] text-white/30">Registry History</h3>
                            {codes.length > 0 && (
                                <span className="text-[10px] font-black text-white/20 bg-white/5 px-2 py-0.5 rounded border border-white/5">{codes.length}</span>
                            )}
                        </div>
                        {renderHistory()}
                    </div>
