...
const DiscountWaiverCard: React.FC<DiscountCardProps> = ({ rules, onNewRule }) => {
    const [isExpanded, setIsExpanded] = useState(false);

    const getCategoryColor = (category: string) => {
        switch (category?.toLowerCase()) {
            case 'merit': return 'text-amber-400 bg-amber-500/10 border-amber-500/20';
            case 'sibling': return 'text-blue-400 bg-blue-500/10 border-blue-500/20';
            case 'staff': return 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20';
            case 'ews': return 'text-pink-400 bg-pink-500/10 border-pink-500/20';
        ...
        <div className="w-full bg-[#12141c] border border-white/5 rounded-[2rem] overflow-hidden transition-all hover:border-white/10 group">
            ...
            >
                ...
                <div className="flex items-center gap-4">
                    <button
                        onClick={(e) => { e.stopPropagation(); onNewRule(); }}
                    ...
                    {rules.length === 0 ? (
                        ...
                        rules.map((rule, idx) => (
                            <div key={idx} className="bg-white/[0.02] border border-white/5 rounded-3xl p-6 relative overflow-hidden hover:bg-white/[0.04] transition-all group/card">
                                ...
                                <h4 className="text-lg font-bold text-white mb-1 relative z-10">{rule.name}</h4>
