    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (toRemove.length > 0) {
                const { error: removeError } = await supabase.rpc('bulk_update_teacher_profiles', {
            ...
            if (toUpdate.length > 0) {
                const { error: updateError } = await supabase.rpc('bulk_update_teacher_profiles', {
                        ...
                        {workloadData.map((d, i) => (
                            <div key={i} className="group">
                                ...
                                <div className="h-3 w-full bg-muted/50 rounded-full overflow-hidden">
                                    <div
                                        className={`h-full rounded-full transition-all duration-1000 ease-out ${d.load > 90 ? 'bg-red-500' : d.load > 75 ? 'bg-amber-500' : 'bg-emerald-500'
                                            }`}
                                        style={{ width: `${d.load}%` }}
                                    ></div>
                                </div>
